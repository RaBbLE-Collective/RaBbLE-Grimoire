...
> - **Phase 4** ✅ `tuned/asusd/asusctl/arbitration.yml` + waybar `power-profile.sh` rewrite + `custom/power-profile` module + AC-unplug udev rule. **Syntax CONFIRMED against asusctl 6.3.8-2.fc43 (2026-07-06):** `asusctl profile set <Profile>` (not `-P`); `asusctl fan-curve --mod-profile <P> --fan <cpu/gpu/mid> --data <30c:0%,...>` + a follow-up `--enable-fan-curves true` per profile (writing `--data` alone does not enable the curve). **Also corrected:** there is no standalone `asusd` package — the daemon binary + `asusd.service` ship inside `asusctl`, so the bogus `asusd` manifest entry (which made `layerctl` fail with "No package asusd available") was removed and `asusd.yml` now only enables the service.
...
**`asusd.yml` / `asusctl.yml`:** install `asusctl` (declared in manifest via `copr:lukenukem/asus-linux`), enable `asusd.service`. **Correction (2026-07-06):** there is no separate `asusd` package — `/usr/bin/asusd` and `asusd.service` ship *inside* `asusctl` (`rpm -ql asusctl`). `asusctl.yml` installs the package; `asusd.yml` only enables the service. The original stub-fill added a spurious `asusd` package + `dnf install asusd` task, which failed the whole hardware role on the first real `layerctl` run — both removed.
