...
- **Aether-themed Gnome DE — exploratory R&D (decided S140).** A testbed to (a) learn theming deterministically and (b) make a more approachable RaBbLE-OS for standard users, with a semi-uniform feel between Hyprland and Gnome (flowing-border + transparency carried into Gnome; harvest what Gnome does well back into Hyprland). **Not a committed ship target** — exploration toward EP2/EP3. Theme work here should feed the deterministic theming plan (bucket D), not fork it. **Implementation started:** opt-in `layer/gnome` Ansible role (SDDM-only, zero GNOME Shell extensions, system-wide dconf-driven Aether GTK/icon defaults) — see → `desktop/RaBbLE-OS-Desktop-Gnome.md`. Pending: VM verification pass (F1-style boot/session-picker/regression checklist in that doc).
