...
**Still open: a deeper startup hang, past the `/Tiertime` fix.** Even with both fixes above, UP Studio's *own* log shows it getting further (past printer-config reads, a `GetVendorMatVersion` check, temp-file cleanup, ending with `Thread_BEGIN_UpdateOtherStatus`) but then stalls on the splash screen indefinitely — reproduced consistently across multiple clean launches, log stops at the identical point every time. Ruled out: the Cetus3D MK2 being disconnected (tested with it plugged in from launch — no change), a hanging network call (`WSALookupServiceBeginW` fails fast with an error, doesn't block), and the CUPS/IPP connection (stays open but idle, not the blocker post-fix). A `fixme:hid:handle_IRP_MN_QUERY_ID` in the Wine trace looked promising (this app's core function is direct USB/HID printer access) but turned out to be ordinary Wine-boot input-device enumeration, unrelated — it only appears in the first ~200 lines of trace, nowhere near the actual stall. No further lead was found without a debugger attach (winedbg/gdb) for a real stack trace of the blocked thread — bigger time investment, not attempted yet. Pick this up here if revisited.
