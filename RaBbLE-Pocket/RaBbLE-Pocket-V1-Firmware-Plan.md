...
First custom firmware build for the Waveshare ESP32-S3-Touch-AMOLED-1.75-B. Planned 2026-08-08, status: **Slices 0-6 verified on real hardware** — scaffold boots, AXP2101 battery/charging + PCF85063 RTC read/validity confirmed, idle screen with eyes/portals/time/battery, settings screen with RTC write, boot animation, and the full Tier 0/1/2 power state machine (click-to-sleep, click-to-wake, 45s auto power-gate, single-click cold wake). Check `firmware/CONTEXT.md` for the current slice. Architecture background: `RaBbLE-Pocket-Architecture.md`. Board spec: `RaBbLE-Pocket-Hardware.md`. Toolchain: `RaBbLE-Pocket-Firmware-BuildFlash.md`.

**Slice 5-6 (power) hardware verification, 2026-08-08**: all four tier transitions confirmed live on the board via serial log + physical clicks. Tier 0→1 (click, display+backlight off, logged) and Tier 1→0 (click within 45s, `Tier 1 -> Tier 0 (wake click, N ms in sleep)`, instant resume, no reboot) both round-tripped cleanly and repeatedly. Tier 1→2 confirmed the hard way: left unclicked, `/dev/ttyACM0` disappeared from the host entirely at the 45s mark — expected, since this board's USB is the ESP32-S3's *native* USB-JTAG-serial peripheral (not an external bridge chip), so cutting the main rail necessarily drops the whole USB device, not just the app. Tier 2→0 then confirmed the plan's one flagged open risk: a **single** PWR click reliably re-powered the rail and produced a fresh cold boot on the *first* attempt (`~3s` felt total, matching the ~2.95s measured floor) — the AXP2101's 128ms PEKEY press-on-time setting does *not* need to have survived the power-gate for this to work, since `rabble_hal_init()` reasserts it on every cold boot regardless; the risk is now closed. One non-fatal rough edge found in passing: an `E (...) lcd_panel.io.i2c: panel_io_i2c_tx_buffer` transmit error logged once, right at a Tier 1 entry — most likely the touch controller's IRQ-driven I2C poll racing the light-sleep transition (CST9217 touch uses `esp_lcd_panel_io_i2c` under the hood). Device recovered fine both times observed; not investigated further this session, flagged for anyone touching touch/sleep interaction later.
...
- **Tier 2→0 wake press duration**: resolved and now hardware-confirmed — `setPowerKeyPressOnTime(XPOWERS_POWERON_128MS)` configured once in `rabble_hal_init()` makes a normal single click sufficient. The previously-flagged open risk (whether this register setting survives the power-gate itself, vs. resetting to a power-on default each time) is **closed**: a single click reliably re-powered the rail on the first real Tier-2 wake attempt (2026-08-08 hardware session) — moot either way, since `rabble_hal_init()` reasserts the setting on every cold boot regardless of what state AXP2101 came back up in.
- **Residual-press false trigger**: a real risk found during implementation, not in the original plan — the same physical click that cold-boots the board from Tier 2 may still be *held down* when `rabble_power_task` starts polling seconds later, which would otherwise be misread as a fresh Tier 0→1 click and immediately re-sleep the device right after boot. Fixed by requiring the task to observe one full button release before it starts treating any press as a real transition (`wait_for_release()` called once at task start, before the main poll loop).
...
6. **Tier 1 light-sleep click-to-sleep/wake** — ✅ verified 2026-08-08: click sleeps (display+backlight off, logged), a second click within 45s wakes instantly with no reboot and no boot-animation replay, log shows exact ms elapsed in sleep.
7. **Tier 2 power-gate (last)** — ✅ verified 2026-08-08: left unclicked for the full 45s, `/dev/ttyACM0` disappeared from the host (rail cut, expected). A single PWR click reliably produced a fresh cold boot (~3s felt total) — resolves the 128ms-press-on-config persistence risk above.
